...
const Footer = ({ data, info, privacyLink = '#', termsAndConditionsPage = '#', locations, socialLinks }: Props) => {
  ...
  const footerSocialItems: FooterSocialItem[] = [
    ...
    },
    {
      ...
      icon: <Icon icon="akar-icons:tiktok-fill" color="white" width="24" height="24" />,
    },
  ];

  const visibleFooterSocialItems = footerSocialItems.filter((item) => Boolean(item.href));
        ...
        <div className="w-90% max-w-1560 mx-auto">
          ...
          <div className="flex flex-col-reverse items-start justify-between mt-12 md:mt-20 gap-y-8 md:flex-row">
            ...
            <a
              ...
              {visibleFooterSocialItems.map((item) => (
                <a
                  key={item.title}
                  title={item.title}
                  href={item.href!}
